...
def ingest_file(file_info, index=0, dataset=None, chart=None, replace=False, skip_cache=False):
    ...
    if os.path.exists(file_location) and skip_cache:
...
def run_conversion_script(script_path: str, dataset_for_conversion, dataset) -> None:
...
def ingest_datasets(
    data: list[DatasetItem], json_file_path: str, replace=False, skip_cache=False
) -> None:
    superuser = User.objects.filter(is_superuser=True).first()
    if superuser is None:
        raise Exception('Please create at least one superuser')
...
def ingest(file_path, replace, clear, skip_cache):
    file_path = Path(DATA_FOLDER, file_path)
    ...
    ingest_datasets(datasets, str(file_path), replace=replace, skip_cache=skip_cache)
